...
?>
    ...
    <tr>
        <td colspan="3"><a id="vv" href="#popup_result" data-icon="delete" data-rel="popup" data-position-to="window" data-transition="fade" class="ui-btn ui-corner-all">로또</a></td>
...
<?
for($i=596;$i>=1;$i--) {
    echo "                        <option value=\"".$i."\">".$i."회</option>";
} ?>

                    </select>
                </td>
                <td><a href="#" id="realbuy" class="ui-btn ui-corner-all">당첨확인</a></td>
            </tr>
        </table>
        <table id="savenum">
        </table>
...
$(function() {
    ...
    // 당첨 내역 확인
    $('#realbuy').click(function() {
        var nbc = $('#nbc option:selected').val();
        var nbbuy = new Array;
        $('#savenum #nbbuy').each(function() {
            if($(this).attr('data-val') == 'Y') {
                nbbuy.push($(this).attr('data-nb'));
            }
        });
        if(nbbuy.length == 0) {
            alert('선택한 로또 번호가 없습니다.');
...
function vvrandom() {
    ...
    var i=0;
    $('.nb').addClass('ui-btn-b');
    function sto() {
        setTimeout(function() { rdc(rd[i]); i++; if(i<6) { sto(); } },500);
    }
    setTimeout(function() { $('.nb').removeClass('ui-btn-b'); sto(); }, 500);
    function rdc(nrd) {
        $('#nb'+nrd).val(nrd).addClass('ui-btn-b');
    }
    cnt = 6;
}
...
// 로또 번호 당첨 확인
function lottobuycheck(cnt, arr) {
    var vvd = $.ajax({
        type : "post",
        url : "/lt/lottobuy",
        data : { cnt : cnt, nbc : arr }
    });
    vvd.done(function(msg) {
        alert(msg);
    });
